...
public class MigrationScreen {
            ...
            try {
                while (true) {
                    ...
                    if (key != null && key.getKeyType() == KeyType.Escape) {
                        synchronized (MigrationScreen.this) {
                            textGraphics.setForegroundColor(TextColor.ANSI.RED);
                            textGraphics.putString(0, startRowForThreads + threadCount + 2,
                                    "Exit requested. Press 'y' to confirm, any other key to resume.");
                            screen.refresh();
                        }
                        KeyStroke confirmKey = screen.readInput();
                        if (confirmKey != null && confirmKey.getCharacter() != null
                                && Character.toLowerCase(confirmKey.getCharacter()) == 'y') {
                            screen.stopScreen();
    ...
    public synchronized void updateGlobal(String phase, long migrated, long total, String eta, String startTime,
            String elapsedTime) {
        if (phase != null)
            this.phase = phase;
        ...
        if (eta != null)
            this.estimatedTimeStr = eta;
        if (startTime != null)
            this.startTimeStr = startTime;
        if (elapsedTime != null)
            this.elapsedTimeStr = elapsedTime;
        redraw();
    }
    ...
    private void redraw() {
        ...
        try {
            ...
            textGraphics.putString(0, 3, String.format(java.util.Locale.forLanguageTag("es-AR"),
                    "Overall Progress: %,d / %,d", migratedRecords, totalRecords));
